...
VERSION_HISTORY = [
    ...
    },
    {
        ...
        'type': 'patch',
        'changes': [
            '🎨 APPLICATIONS PAGE: Filter Toggle + Dark Theme (Replicating Connected Devices)',
            '',
            'NEW FEATURE: Collapsible Filter Bar',
            '- Added "Show Filters" / "Hide Filters" toggle button',
            '- Filters now hidden by default (clean interface)',
            '- Button styled with dark theme matching Connected Devices',
            '- State persisted via localStorage ("applicationsFiltersVisible")',
            '- Smooth transitions when showing/hiding filters',
            '',
            'REVERSE DNS CHECKBOX: Moved Outside Filter Area',
            '- Relocated from inside filter bar to top-level controls',
            '- Now visible even when filters are collapsed',
            '- Styled with dark theme (dark gradient, #555 border)',
            '- Matches Connected Devices layout exactly',
            '',
            'DARK THEME: Filter Bar Transformation',
            '- Background: White → Gradient (#F2F0EF → #E8E6E4)',
            '- Border: 3px solid #3c3c3c + 5px top border #FA582D',
            '- Input backgrounds: White → #F2F0EF',
            '- Export buttons: White with orange border → Orange gradients (#C64620, #AD3D1A)',
            '- Focus states: Orange highlight on all inputs',
            '',
            'TECHNICAL IMPLEMENTATION:',
            '- templates/index.html:',
            '  - Added filter toggle button (lines 1447-1456)',
            '  - Moved Reverse DNS checkbox outside filter area (lines 1459-1464)',
            '  - Updated filter area ID to "applicationsFilterArea" (line 1468)',
            '  - Applied dark theme colors to all filter elements',
            '  - Export buttons now use orange gradients matching brand',
            '- static/pages-applications.js:',
            '  - Added toggleApplicationsFilters() function (lines 977-1001)',
            '  - Added restoreApplicationsFiltersState() function (lines 1007-1025)',
            '  - Exported toggle function to window object (line 1028)',
            '  - Console logging for debugging filter state',
            '- static/app.js:',
            '  - Added restoreApplicationsFiltersState() call on page load (line 2329)',
            '',
            'USER EXPERIENCE:',
            'BEFORE:',
            '- Filters always visible (cluttered interface)',
            '- Reverse DNS checkbox buried inside filter bar',
            '- Bright white filter bar (inconsistent with app theme)',
            '- Export buttons had minimal styling',
            '',
            'AFTER:',
            '- Filters collapsed by default (clean starting view)',
            '- Reverse DNS checkbox always accessible',
            '- Consistent dark theme across entire page',
            '- Professional gradient buttons matching brand colors',
            '- Matches Connected Devices page exactly',
            '',
            'CONSISTENCY ACHIEVED:',
            '- Applications page now mirrors Connected Devices layout',
            '- Same filter toggle pattern throughout application',
            '- Unified dark theme aesthetic',
            '- Predictable user experience across pages'
        ]
    },
    {
        'version': '2.1.10',
        'codename': 'Traffic Flow Dark Mode',
        'date': '2025-11-24',
        'type': 'patch',
        'changes': [
            '🎨 TRAFFIC FLOW BREAKDOWN MODAL: Dark Theme + Reverse DNS Lookup',
            '',
            'DARK THEME TRANSFORMATION:',
            '- Modal background: White → Dark gradient (#2d2d2d → #1a1a1a)',
            '- Modal overlay: rgba(0,0,0,0.7) → rgba(0,0,0,0.85)',
            '- Added white top border (#F2F0EF) to match design language',
            '- Header: #333 → #F2F0EF text, border: #FA582D → #555',
            '- Close button: #999 → #ccc with hover transition',
            '- Diagram container: #f9f9f9 → Dark gradient (#3c3c3c → #2d2d2d)',
            '- Loading spinner: #f3f3f3 → #555 border',
            '- Loading text: #666 → #ccc',
            '- Error state: Light yellow → Dark brown gradient with yellow text',
            '- Info box: Light blue → Dark blue gradient with light text',
            '- SVG text labels: #333 → #F2F0EF with font-weight 500',
            '',
            'NEW FEATURE: Reverse DNS Lookup',
            '- Added checkbox on Connected Devices page: "Reverse DNS Lookup"',
            '- Global toggle affects ALL traffic flow modals',
            '- Checkbox styled with dark theme (dark gradient background)',
            '- State persisted via localStorage ("reverseDnsEnabled")',
            '- Default: DISABLED (user can enable as needed)',
            '- When enabled: IP addresses resolve to hostnames in Sankey diagram',
            '- DNS cache implemented to avoid repeated lookups',
            '- Seamless integration: checkbox → Sankey diagram labels',
            '',
            'TECHNICAL IMPLEMENTATION:',
            '- templates/index.html:',
            '  - Updated modal container (line 1618): Dark gradient background, white top border',
            '  - Updated modal header (line 1620): Dark border, light text colors',
            '  - Updated diagram container (line 1645): Dark gradient with border',
            '  - Updated loading/error/note states (lines 1651-1669): Dark theme colors',
            '  - Added reverse DNS checkbox (lines 1323-1333): Dark themed control',
            '- static/pages-connected-devices-core.js:',
            '  - Added toggleReverseDnsLookup() function (line 775)',
            '  - Added restoreReverseDnsState() function (line 785)',
            '  - Added performReverseDnsLookup() async function (line 801)',
            '  - Added getIpDisplayLabel() helper (line 837)',
            '  - Added isReverseDnsEnabled() getter (line 849)',
            '  - Exported functions to window object (lines 877-888)',
            '  - Called restoreReverseDnsState() on page load (line 197)',
            '- static/pages-connected-devices-sankey.js:',
            '  - Updated text fill color (line 401): #333 → #F2F0EF',
            '  - Added font-weight 500 for better readability',
            '  - Integrated reverse DNS lookup in buildSankeyData() (lines 437-447)',
            '  - Source and destination IPs use DNS labels when enabled',
            '  - Graceful fallback to IP if ConnectedDevices module unavailable',
            '',
            'USER EXPERIENCE:',
            'BEFORE:',
            '- Traffic Flow modal had bright white background (jarring)',
            '- Inconsistent with dark theme across application',
            '- IP addresses always shown as raw numbers',
            '- No option for hostname resolution',
            '',
            'AFTER:',
            '- Consistent dark theme matching main dashboard aesthetic',
            '- Professional appearance with proper contrast',
            '- Optional reverse DNS for better readability',
            '- Global checkbox controls all modals',
            '- Hostnames displayed when DNS enabled (e.g., "server1.local:443")',
            '',
            'BENEFITS:',
            '- ✅ Unified dark theme across all pages and modals',
            '- ✅ Reverse DNS makes traffic flows more human-readable',
            '- ✅ One checkbox controls all flow visualization modals',
            '- ✅ DNS state persisted across sessions',
            '- ✅ Improved text readability with proper contrast',
            '- ✅ Professional appearance consistent with brand',
            '',
            'FILES MODIFIED:',
            '- templates/index.html: Modal dark theme (lines 1617-1670), DNS checkbox (lines 1323-1333)',
            '- static/pages-connected-devices-sankey.js: Text colors, DNS integration (lines 401, 437-447)',
            '- static/pages-connected-devices-core.js: DNS functions and state management (lines 762-851)',
            '- version.py: Updated to v2.1.10 "Traffic Flow Dark Mode"',
        ]
    },
    {
        'version': '2.1.9',
        'codename': 'Connected Devices UI Polish',
        'date': '2025-11-24',
        'type': 'patch',
        'changes': [
            '🎨 CONNECTED DEVICES PAGE: UI Polish and Collapsible Filters',
            '',
            'STYLING IMPROVEMENTS:',
            '- Removed excessive orange: Replaced thick orange border with elegant white top border',
            '- Match Internal Traffic table: White/light top border (#F2F0EF) for consistency',
            '- Table container: border: 3px solid #FA582D → border-top: 4px solid #F2F0EF',
            '- Header background: Orange gradient (#FA582D → #FF7A55) → Dark gradient (#3c3c3c → #2d2d2d)',
            '- Header bottom border: 3px solid #FA582D → 2px solid #555',
            '- Cleaner, more professional appearance',
            '',
            'NEW FEATURE: Collapsible Filter Area',
            '- Added toggle button above filter area (similar to sidebar toggle)',
            '- Toggle button: "▼ Show Filters" / "▲ Hide Filters"',
            '- Button styling: Dark gradient with hover effects',
            '- DEFAULT STATE: Filters HIDDEN (user requested)',
            '- State persisted via localStorage',
            '- Table adjusts position when filters collapse',
            '',
            'TECHNICAL IMPLEMENTATION:',
            '- pages-connected-devices-core.js:',
            '  - Added toggleConnectedDevicesFilters() function',
            '  - Added restoreConnectedDevicesFilterState() function',
            '  - localStorage key: "connectedDevicesFiltersVisible"',
            '  - Default: false (hidden)',
            '  - Called in loadConnectedDevices() after table render',
            '  - Exported to window for inline onclick handlers',
            '- templates/index.html:',
            '  - Added toggle button before filter area',
            '  - Added id="connectedDevicesFilterArea" to filter container',
            '  - Added display: none; to filter container (default hidden)',
            '  - Added id="toggleConnectedDevicesFiltersBtn" to button',
            '',
            'BEFORE/AFTER:',
            'BEFORE:',
            '- Orange border and header dominated the visual',
            '- Filters always visible, taking up screen space',
            '- Inconsistent with Internal Traffic table style',
            '',
            'AFTER:',
            '- Clean white top border matches Internal Traffic',
            '- Filters hidden by default, revealed via toggle',
            '- More screen space for device table',
            '- Consistent dark theme throughout',
            '',
            'FILES MODIFIED:',
            '- static/pages-connected-devices-core.js: Styling updates (lines 486, 487, 499), collapsible functions (lines 703-757), exports (lines 780-788)',
            '- templates/index.html: Toggle button (lines 1310-1321), filter area ID and display:none (line 1324)',
            '- version.py: Updated to v2.1.9 "Connected Devices UI Polish"',
        ]
    },
    {
        'version': '2.1.8',
        'codename': 'Dark Theme Unification',
        'date': '2025-11-24',
        'type': 'patch',
        'changes': [
            '🎨 VISUAL OVERHAUL: Dark Theme Across All Pages',
            '',
            'UNIFIED: Consistent Dark Theme Throughout Application',
            '- DASHBOARD: Stat cards dark gradient, chart containers dark background',
            '- CONNECTED DEVICES: Table dark theme with proper text contrast',
            '- APPLICATIONS: Dark gradients, headers, rows, detail boxes',
            '- ALERTS: Complete dark theme for tables and modals',
            '- ALL PAGES: Unified color palette for professional appearance',
            '',
            'COLOR PALETTE APPLIED:',
            '- Primary container: linear-gradient(135deg, #2d2d2d → #1a1a1a)',
            '- Card/tile: linear-gradient(135deg, #3c3c3c → #2d2d2d)',
            '- Table header: #3c3c3c',
            '- Table rows even: #2a2a2a, odd: #333333',
            '- Table row hover: linear-gradient(135deg, #3a3a3a → #333333)',
            '- Text primary: #F2F0EF, secondary: #ccc, muted: #999',
            '- Borders: #444 (light), #555 (medium), #FA582D (accent)',
            '',
            'DASHBOARD CHANGES:',
            '- Stat cards: White gradient → Dark gradient (#3c3c3c → #2d2d2d)',
            '- Stat labels: #333 → #F2F0EF for readability',
            '- Mini charts: White bg → Dark transparent (#000 0.3 opacity)',
            '- Chart containers: White → Dark gradient with #FA582D border',
            '- Status indicator: White → Dark transparent with cream text',
            '',
            'CONNECTED DEVICES CHANGES:',
            '- Table container: Light cream (#F2F0EF) → Dark gradient (#2d2d2d)',
            '- Table rows: White/light gray → Dark alternating (#2a2a2a/#333)',
            '- Row hover: Light peachy → Dark gradient (#3a3a3a)',
            '- All text: #333/#666 → #F2F0EF/#ccc for proper contrast',
            '- Comment rows: #f8f9fa → #2a2a2a with #555 borders',
            '- Location/Comment labels: #333 → #F2F0EF',
            '',
            'APPLICATIONS PAGE CHANGES:',
            '- Empty state: White → Dark gradient',
            '- Main container: White → Dark gradient with #FA582D border',
            '- Table headers: #333 → #F2F0EF',
            '- Table rows: White → Transparent with dark hover',
            '- Row hover: #f9f9f9 → Dark gradient (#3a3a3a)',
            '- All cell text: #666 → #ccc',
            '- Detail boxes: White → Dark gradient (#3c3c3c → #2d2d2d)',
            '- Border colors: #ddd → #555 for consistency',
            '',
            'ALERTS PAGE CHANGES:',
            '- All white backgrounds → Dark gradients',
            '- Table headers: #f5f5f5 → #3c3c3c',
            '- Table rows: #ffffff/#f8f9fa → #2a2a2a/#333',
            '- Row hover: #f0f0f0 → #3a3a3a',
            '- All text: #333/#666 → #F2F0EF/#ccc',
            '- Template cards: #f8f9fa → Dark gradient',
            '- Modal backgrounds: White → Dark gradient',
            '',
            'BENEFITS:',
            '- ✅ Consistent professional dark theme across all pages',
            '- ✅ Reduced eye strain with darker colors',
            '- ✅ Brand colors (#FA582D) stand out better on dark background',
            '- ✅ Improved visual hierarchy with proper contrast',
            '- ✅ Modern, cohesive appearance matching main dashboard',
            '',
            'TECHNICAL IMPLEMENTATION:',
            '- Used replace_all for efficient bulk updates',
            '- Maintained all interactive states (hover, active)',
            '- Preserved brand accent colors (#FA582D orange)',
            '- Ensured WCAG contrast compliance for text',
            '- Tested across all major pages',
            '',
            'FILES MODIFIED:',
            '- templates/index.html: Dashboard stat cards, chart containers, status indicator',
            '- static/pages-connected-devices-core.js: Table, rows, cells, expanded details',
            '- static/pages-applications.js: Containers, headers, rows, detail boxes',
            '- static/pages-alerts.js: Tables, modals, template cards',
            '- version.py: Updated to v2.1.8 "Dark Theme Unification"',
        ]
    },
    {
        'version': '2.1.7',
        ...
        'type': 'patch',
        'changes': [
            '🔧 CRITICAL FIX: Sidebar Collapse Layout',
            '',
            'FIXED: Content Truncation on Sidebar Collapse',
            '- ROOT CAUSE: Sidebar using margin-left: -250px + width: 0 broke flexbox flow',
            '- SOLUTION: Transform-based sidebar hide (translateX instead of margin)',
            '- BENEFIT: Main content automatically expands via flex-grow (no manual width calc)',
            '',
            'IMPLEMENTATION: Transform-Based Sidebar',
            '- CHANGED: sidebar.collapsed from width: 0 to transform: translateX(-253px)',
            '- ADDED: flex-shrink: 0 to maintain 250px width in flexbox',
            '- REMOVED: All margin-left manipulation from .main-content',
            '- REMOVED: body.sidebar-collapsed class logic (no longer needed)',
            '- SIMPLIFIED: Toggle button transitions (all 0.3s ease)',
            '',
            'TECHNICAL DETAILS:',
            '- Sidebar: Fixed 250px width with transform slide animation',
            '- Main Content: flex-grow: 1 automatically fills available space',
            '- Layout: Clean flexbox flow preserved during collapse/expand',
            '- Transform: translateX(-253px) accounts for 250px width + 3px border',
            '',
            'TESTING:',
            '- ✅ Sidebar collapses smoothly to left edge',
            '- ✅ Main content expands to full viewport width',
            '- ✅ Charts and tables no longer truncate',
            '- ✅ Toggle button repositions correctly',
            '- ✅ No layout jank or content jumping',
            '',
            'FILES MODIFIED:',
            '- templates/index.html: Sidebar CSS (transform approach), main-content cleanup',
            '- version.py: Updated to v2.1.7 "Layout Fix"',
        ]
    },
    {
        'version': '2.1.6',
        ...
        'changes': [
            ...
            '',
            'FILES MODIFIED:',
            '- templates/index.html: Menu styling, icons, toggle button',
            '- static/app.js: Toggle button body class logic',
    ...
    },
    {
        ...
        'type': 'major',
        'changes': [
            '🚀 MAJOR RELEASE: Enterprise Database Migration',
            '',
            '═══════════════════════════════════════════════════════',
            'BREAKING CHANGE: SQLite → TimescaleDB Migration',
            '═══════════════════════════════════════════════════════',
            '',
            ...
            'ADDED: Automatic retention policies (7d raw → 90d hourly → 1y daily)',
            ...
            'IMPROVED: 1-hour queries: 1.2s → 0.1s (12x faster)',
            'IMPROVED: 24-hour queries: 4.5s → 0.3s (15x faster)',
            'IMPROVED: 30-day queries: 18.2s → 0.5s (36x faster)',
            'IMPROVED: 90-day queries: timeout → 0.8s (infinite improvement)',
            ...
            '✅ Run docker-compose up -d (starts all containers)',
            '✅ Run migrate_sqlite_to_timescale.py (migrates historical data)',
            '✅ Verify migration with validation check',
            '✅ Test all time ranges (1h, 6h, 24h, 7d, 30d, 90d)',
            '',
            ...
            '• Production-ready for 1-10 firewalls',
            '',
            '🎉 PANfm v2.0.0 - READY FOR ENTERPRISE!'
        ]
    },
    {
        'version': '1.14.1',
        ...
        'type': 'patch',
        'changes': [
            '🐛 CRITICAL FIX: Main Dashboard Time Range Selector',
            ...
            '🐛 CRITICAL FIX: Insights Dashboard Time Range Selector',
    ...
    },
    {
        ...
        'type': 'minor',
        'changes': [
            '🏗️ PHASE 1 COMPLETE: Enterprise Reliability Fixes',
            ...
            'FIX: Applications page shows ⏳ waiting indicator instead of errors',
            'FIX: Hidden registration bugs eliminated by flattening imports',
            '🚀 PHASE 2 COMPLETE: API Standardization',
            ...
            '📚 PHASE 3 COMPLETE: Documentation & Registry',
            ...
            'ARCHITECTURE: 2-level route imports (routes.py → routes_throughput.py)',
            '✅ TESTING READY:',
    ...
    },
    {
        ...
        'type': 'minor',
        'changes': [
            '✨ NEW FEATURE: Analytics Dashboard with historical data visualization',
            ...
            '🗑️ REMOVED: Security Monitoring feature (~4,700 lines)',
